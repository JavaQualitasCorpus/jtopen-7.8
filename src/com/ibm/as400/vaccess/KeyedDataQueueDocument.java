...
//                                                                             
// Filename: KeyedDataQueueDocument.java
//                                                                             
...
/**
The KeyedDataQueueDocument class represents an underlying model
for text components, where the text is entries from a keyed
...
<p>KeyedDataQueueDocument objects generate the following events:
<ul>
    <li>DataQueueEvent
    <li>DocumentEvent
    <li>ErrorEvent
...
**/
    ...
    {
        dataQueue_ = new KeyedDataQueue (system, path);
        initializeTransient ();
    }
...
**/
    public void peek (String key)
    {
        ...
        try {
            ...
            // Peek the data queue.
            DataQueueEntry entry = dataQueue_.peek (key);
            String contents;
            if (entry != null)
...
@param searchType   The type of comparison to use to determine if a key
                    is a match. Valid values are EQ (equal), NE (not
                    equal), LT (less than), LE (less than or equal), GT
                    (greater than), and GE (greater than or equal).
**/
    public void peek (String key, int wait, String searchType)
    {
        ...
        try {
            ...
            // Peek the data queue.
            DataQueueEntry entry = dataQueue_.peek (key, wait, searchType);
            String contents;
            if (entry != null)
                contents = entry.getString ();
            else
...
**/
    public void read (String key)
    {
        ...
        try {
            ...
            // Peek the data queue.
            DataQueueEntry entry = dataQueue_.read (key);
            String contents;
            if (entry != null)
                contents = entry.getString ();
            else
...
**/
    public void read (String key, int wait, String searchType)
    {
        ...
        try {
            ...
            // Peek the data queue.
            DataQueueEntry entry = dataQueue_.read (key, wait, searchType);
            String contents;
            if (entry != null)
                contents = entry.getString ();
            else
