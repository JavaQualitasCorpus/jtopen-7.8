...
implements DBData
{
    ...
    private int[]   totalDataLengthsFromHost_  = null; //@array total length of data portion in stream
    private int[]   arrayDataLengthsFromHost_  = null; //@array length of each data element in array
    ...
    **/
    ...
    public void overlay (byte[] rawBytes, int offset)
    {
        ...
        if (rowCount_ == -1) {
            ...
            totalDataLengthsFromHost_ = new int[columnCount_];  //one per column
            arrayDataLengthsFromHost_ = new int[columnCount_];  //one per column @G2A
            ...
            for(int i = 0; i < columnCount_; i++){
                ...
                if(descType == (short)0x9911){
                    ...
                    if(isNull){
                        ...
                        dataOffsetsFromHost_[i] = (i == 0 ? 0 : dataOffsetsFromHost_[i-1] + totalDataLengthsFromHost_[i-1]); //keep same as prev since null arrays have no data (so we can later keep a running total)(ie no -1 values)
                        totalDataLengthsFromHost_[i] = arrayLen * dataLen;
                        arrayDataLengthsFromHost_[i] = dataLen; /*@G2A*/
                    ...
                    dataOffsetsFromHost_[i] = (i == 0 ? 0 : dataOffsetsFromHost_[i-1] + totalDataLengthsFromHost_[i-1]); //keep same as prev since null arrays have no data (so we can later keep a running total)(ie no -1 values)
                    totalDataLengthsFromHost_[i] = dataLen;
                    arrayDataLengthsFromHost_[i] = 0; /*@G2A*/
    ...
    /**
     * Returns the size footprint of data in column.
     * For arrays, this is arrayLen * arrayDataLen
     */
     /*@G2A*/
